...
def make_plot():
    ...
    plot.xlabel(r"$k$ $[\Omega / v_0]$", fontsize = fontsize)
    plot.ylabel(r"$s$ $[\Omega]$", fontsize = fontsize)
    plot.title("Latter+ 2010: Figure 1 (reproduced)", fontsize = fontsize + 1)

    plot.xscale("log")

    plot.savefig("latter2010-fig1-reproduced-from-matrix-with-va2.png", bbox_inches = "tight")
    plot.show()
