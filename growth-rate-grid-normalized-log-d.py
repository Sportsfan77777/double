...
def make_plot(version = None, Reynolds_number = 1.0e8, show = False):
    ...
    beta2 = 5e4; mri_x2 = np.sqrt(2) * np.power(beta2, -0.5)
...
#    plot.text(0.3 * mri_x1, y_mri_text, r"$\beta = $", fontsize = fontsize - 5)
    ...
    plot.text(0.5 * mri_x2, y_mri_text, r"$\beta = 5 \times 10^{4}$", fontsize = fontsize - 5, horizontalalignment = "center")
    plot.text(0.9 * mri_x3, y_mri_text, r"$10^{3}$", fontsize = fontsize - 5)

    if version is None:
       plot.savefig("latter2010-v0-growth-rate-grid-normalized-log-formal-d.png", bbox_inches = "tight")
    else:
       plot.savefig("v%04d_latter2010-v0-growth-rate-grid-normalized-log-formal-d.png" % version, bbox_inches = "tight")
