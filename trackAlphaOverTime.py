...
def get_alpha(args):
    ...
    vx_vy = np.abs(vx_data[0][0] * vy_data[0][0])
...
def make_plot(show = False):
    ...
    p.dump(x, open("timeA-N10.p", "wb"))
    p.dump(y, open("alpha-N10.p", "wb"))
    ...
    if version is None:
        save_fn = "%s/averagedAlphaOverTime-%s.png" % (save_directory, cwd)
    else:
        save_fn = "%s/v%04d_averagedAlphaOverTime-%s.png" % (save_directory, version, cwd)
