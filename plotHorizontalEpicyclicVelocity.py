...
def make_plot(frame, show = False):
    ...
    cbar.set_label(r"$v_\mathrm{A}$", fontsize = fontsize, rotation = 270, labelpad = 25)

    # Save, Show, and Close
    if version is None:
        save_fn = "%s/epicyclicVelocityMap_%04d.png" % (save_directory, frame)
    else:
        save_fn = "%s/v%04d_epicyclicVelocityMap_%04d.png" % (save_directory, version, frame)
