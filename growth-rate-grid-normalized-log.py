...
def make_plot(version = None, Reynolds_number = 1.0e8, show = False):
    ...
    beta2 = 1e4; mri_x2 = np.sqrt(2) * np.power(beta2, -0.5)
    ...
    rddi_x = 0.03
    y_label = 2e-5 * max(y)
    plot.text(7.0 * min(x), y_label, "COS", verticalalignment = "center", fontsize = fontsize + 10)
...
#    plot.text(0.3 * mri_x1, y_mri_text, r"$\beta = $", fontsize = fontsize - 5)
    if N_squared <= -0.10:
      ...
      plot.text(1.0 * mri_x2, y_mri_text, r"$\beta = 10^{4}$", fontsize = fontsize - 5, horizontalalignment = "center")
      plot.text(0.9 * mri_x3, y_mri_text, r"$10^{3}$", fontsize = fontsize - 5)
    else:
      plot.text(1.0 * mri_x2, 5.0 * y_mri_text, "MRI cutoffs with", fontsize = fontsize - 5, horizontalalignment = "center")
      plot.text(1.1 * mri_x1, y_mri_text, r"$\beta = 10^{5}$", fontsize = fontsize - 5, horizontalalignment = "right")
      plot.text(1.0 * mri_x2, y_mri_text, r"$10^{4}$", fontsize = fontsize - 5, horizontalalignment = "center")
      plot.text(0.9 * mri_x3, y_mri_text, r"$10^{3}$", fontsize = fontsize - 5)
...
num_Reynolds = 8
Reynolds_numbers = np.logspace(4, 11, num_Reynolds)
