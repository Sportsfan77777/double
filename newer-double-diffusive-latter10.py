...
beta = 2.0 # Plasma beta parameter for (inverse) vertical field strength
...
big_lambda = 1.0 #1.0e16

nuM = 0 #Omega0 * Hg**2 / ReynoldsM # Resistivity
...
#Bz0 = np.sqrt(va2 * mu0 * rhog0) # Equilibrium vertical field strength
#print ("Alfven: %f %f" % (alfven_velocity_squared, va2))
...
kz_mri = 1.0 / np.sqrt(alfven_velocity_squared)
...
if pert == 'eigen':
    ...
    #Lx, Ly, Lz = lambda_x, lambda_z, lambda_z
    Lx, Ly, Lz = lambda_x, lambda_x, lambda_z

else:
    #Lbox = 2.0 * np.pi * Hg
    lambda_z = 2.0 * np.pi / np.abs(kz_mri)
    Lbox = 2.0 * lambda_z
    #Lx, Ly, Lz = Lbox, Lbox, Lbox
    Lx, Ly, Lz = Lbox, Lbox, lambda_z
...
timestepper = d3.RK222 # RK443 or RK222
cfl_number = 0.8 # 0.2
min_timestep = 1e-4 / Omega0
max_timestep = 1e-1 / Omega0

period = 2.0 * np.pi
stop_sim_time = 600 # in orbits
...
### Bases ###
coords = d3.CartesianCoordinates('z', 'x', 'y')
...
# Fields
u = dist.VectorField(coords, name='u', bases=(z_basis,x_basis))
#uy = dist.Field(name='uy', bases=(x_basis,z_basis))
theta = dist.Field(name='theta', bases=(z_basis,x_basis))
tau_P = dist.Field(name='tau_P')
p = dist.Field(name='p', bases=(z_basis,x_basis))

A = dist.VectorField(coords, name = 'A', bases = (z_basis, x_basis)) # Meridional Mangetic Vector Potential
#Ay = dist.Field(name = 'Ay', bases = (x_basis, z_basis)) # Azimuthal Magnetic Vector Potential
phi = dist.Field(name = 'phi', bases = (z_basis, x_basis))
...
problem.add_equation("dt(A) - (3.0/2.0)*Omega0*Ay*ex - eta*lap(A) - grad(phi) = vcrossB + vgx0*DBy*ez - vgx0*DBz")
#problem.add_equation("dt(A) - (3.0/2.0)*Omega0*Ay*ex - eta*lap(A) - grad(phi) = vcrossBxz + vgx0*DBy*ez")
#problem.add_equation("dt(Ay) - eta*lapAy = vcrossBy - vgx0*DBz")
...
class OneFluidMatrices:
    ...
    def Latter2010(self):
        ...
        matrix_a[0] = np.array([-dissipation, 2, ikz, 0, -N_squared / omega_squared])
        matrix_a[1] = np.array([-0.5, -dissipation, 0, ikz, 0])
        matrix_a[2] = np.array([ikz, 0, -ksq / big_lambda - dissipationM, 0, 0])
        matrix_a[3] = np.array([0, ikz, omega_power, -ksq / big_lambda - dissipationM, 0])
    ...
    def Latter2010xz(self):
        ...
        matrix_a[4] = np.array([0, ikz, 0, 0, -ksq / big_lambda, 0, 0, 0])
        matrix_a[5] = np.array([0, 0, ikz, 0, omega_power, -ksq / big_lambda, 0, 0])
        matrix_a[6] = np.array([0, 0, 0, ikz, 0, 0, -ksq / big_lambda, 0])
        matrix_a[7] = np.array([0, 1, 0, 0, 0, 0, 0, -q * ksq / big_lambda])

        return (matrix_a, matrix_b)

    def Latter2010xz_v0_wrong(self):
        kx = self.kx; kz = self.kz
        ikx = 1j * kx; ikz = 1j * kz
        ksq = self.kx**2 + self.kz**2
...
if restart == None: #Fresh run
    if pert == 'eigen':
        ...
        u['g'][0] = np.real(duz * expik)
        u['g'][1] = np.real(dux * expik)
        u['g'][2] = np.real(duy * expik)
        p['g'] = np.real(dp * expik)
        theta['g'] = np.real(dtheta * expik)

        A['g'][0] = np.real(dAz * expik)
        A['g'][1] = np.real(dAx * expik)
        A['g'][2] = np.real(dAy * expik)

    elif pert == 'random':
        # Random perturbation in vgy
        u.fill_random('g', seed = 42, distribution = 'normal', scale = pert_amp * cs)
        u.low_pass_filter(scales = low_pass_scales)
        u['g'][0] = 0.0 # All equilibrium velocities are zero
        u['g'][1] = 0.0 # All equilibrium velocities are zero
        p['g'] = 0.0
...
snapshots.add_task(theta, name = 'theta', scales = OutputScale)
snapshots.add_task(B@ex, name = 'Bx', scales = OutputScale)
snapshots.add_task(B@ey, name = 'By', scales = OutputScale)
snapshots.add_task(B@ez - 1, name = 'Bz', scales = OutputScale)
...
flow.add_property(ux * theta, name = 'duxtheta')
flow.add_property(np.sqrt(Bx**2 + By**2 + (Bz-1.0)**2), name = 'dB2')
flow.add_property(Bx**2, name = 'dBx2')
flow.add_property(By**2, name = 'dBy2')
flow.add_property((Bz-1.0)**2, name = 'dBz2')
flow.add_property(Bx * By, name = 'dBxdBy')

# Prep for analysis output (only first CPU's job)
if rank == 0:
    if restart == None: #Fresh run
        output = open('analysis.txt', 'w')
        outputB = open('magnetic_analysis.txt', 'w')
    else:
        output = open('analysis.txt', 'a')
        outputB = open('magnetic_analysis.txt', 'a')
comm.Barrier()
...
try:
    ...
    while solver.proceed:
        ...
        if solver.sim_time >= log_time:
            ...
            amf = flow.grid_average('duxduy')
            hf = flow.grid_average('duxtheta')
            rms_dB  = np.sqrt(flow.grid_average('dB2'))
            rms_dBx = np.sqrt(flow.grid_average('dBx2'))
            rms_dBy = np.sqrt(flow.grid_average('dBy2'))
            rms_dBz = np.sqrt(flow.grid_average('dBz2'))
            maxwell = flow.grid_average('dBxdBy')

            #write to analysis by CPU 0
            if rank == 0:
                output.write("{0:9.6e}, {1:9.6e}, {2:9.6e}, {3:9.6e}, {4:9.6e}, {5:9.6e}, {6:9.6e}, {7:9.6e}".format(solver.sim_time, max_du, rms_du, rms_dux, rms_duy, rms_duz, amf, hf)+"\n")
                output.flush()

                outputB.write("{0:9.6e}, {1:9.6e}, {2:9.6e}, {3:9.6e}, {4:9.6e}, {5:9.6e}".format(solver.sim_time, rms_dB, rms_dBx, rms_dBy, rms_dBz, maxwell)+"\n")
                outputB.flush()
            comm.Barrier()
