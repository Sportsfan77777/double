...
def get_flux(args):
    ...
    theta_data    = output['tasks']["theta"]

    vx_times_theta = np.multiply(vx_data, theta_data)

    flux = np.average(vx_times_theta, axis = 0)
...
plot.ylabel(r'$<v_\mathrm{x} \theta>$', fontsize = fontsize)
plot.title('Heat Flux', fontsize = fontsize + 1)

cwd = os.getcwd().split("/")[-1]
plot.savefig("heat_flux-%s.png" % cwd, bbox_inches = 'tight')
