...
def get_growth_rates(big_lambda = 1.0e16, N_squared = -0.1, big_lambda_H = 0.2, , big_lambda_A = 0.2): 
...
def make_plot(hall_i, hall_effect):
    ...
    plot.title("Latter+ 2010: Figure 1 (with Hall Ha = %.4f)" % hall_effect, fontsize = fontsize + 1)

    plot.xscale("log")

    plot.savefig("latter2010-fig1-from-matrix-with-hall-%04d.png" % hall_i)
    plot.show()
