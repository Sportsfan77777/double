...
def get_growth_rates(big_lambda = 1.0e16, N_squared = -0.1, big_lambda_H = 1.0e8, big_lambda_A = 0.2): 
    ...
    for i, k in enumerate(ks):
        ...
        matrix[2, 0] = I * k
        matrix[2, 1] = 0
        matrix[2, 2] = -np.power(k, 2.0) / big_lambda + np.power(k, 2.0) / big_lambda_A
        ...
        matrix[3, 3] = -np.power(k, 2.0) / big_lambda + np.power(k, 2.0) / big_lambda_A
        matrix[3, 4] = 0
...
def make_plot(hall_i, hall_effect):
    ...
    ks, growth_rates1 = get_growth_rates(big_lambda = 1.0e10, N_squared = -0.1, big_lambda_A = hall_effect)
    ks, growth_rates2 = get_growth_rates(big_lambda = 1.0, N_squared = 0.0, big_lambda_A = hall_effect)
    ks, growth_rates3 = get_growth_rates(big_lambda = 1.0, N_squared = -0.1, big_lambda_A = hall_effect)

    ks, growth_rates4 = get_growth_rates(big_lambda = 1.0e10, N_squared = 0.1, big_lambda_A = hall_effect)
    ks, growth_rates5 = get_growth_rates(big_lambda = 1.0e10, N_squared = 0.0, big_lambda_A = hall_effect)
    ...
    plot.plot(x, y5, c = 'k', linewidth = linewidth, linestyle = "--", label = r"$\Lambda = \infty, N^2 = 0$")
    ...
    plot.title("Latter+ 2010: Figure 1 (with Am = %.4f)" % hall_effect, fontsize = fontsize + 1)

    plot.xscale("log")

    plot.savefig("latter2010-fig1-from-matrix-with-hall-and-ambi-%04d.png" % hall_i, bbox_inches = 'tight')
    plot.show()
